...
function useTheme() {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { setting, setSetting } = useSetting() as any;
...
export function ThemeToggle({ panelClassName = 'mt-4' }) {
  ...
  return (
    <Listbox value={setting} onChange={setSetting}>
      ...
      <Listbox.Options
        className={clsx(
          'absolute z-50 top-full right-16 bg-white rounded-lg ring-1 ring-slate-900/10 shadow-lg overflow-hidden w-36 py-1 text-sm text-slate-700 font-semibold dark:bg-slate-800 dark:ring-0 dark:highlight-white/5 dark:text-slate-300',
          panelClassName
        )}
      >
